...
当前阶段不触发采集、不生成快照、不实现搜索、筛选、排序或复杂分页。
...
保留 List Reference Page 的正式视觉结构，但本次不实现对应业务能力。
...
- 列表不实现复杂分页、排序、搜索或筛选；其中排序规则固定为 API 的 `created_at DESC, id DESC`，不提供用户排序操作。
- 列表不触发 Playwright。
- 列表不生成快照。
