...
13. 作为用户，我希望在没有竞品组且没有未分组竞品时看到明确空状态，并能直接新增竞品组。
14. 作为用户，我希望从页面顶部新增竞品组，并使用我方商品型号作为组名，以便添加竞品时选择它。
15. 作为用户，我希望创建时输入会被 trim、不能为空、最长 64 个字符且名称唯一，以便型号数据保持稳定。
16. 作为用户，我希望创建失败时看到明确的“竞品组”语义错误，同时由输入提示说明组名使用商品型号。
17. 作为用户，我希望在 AddDialog 中看到“竞品组”“新增竞品组”和“输入商品型号作为组名，例如 A19”，以便功能语义清晰，同时保留型号命名规则。
...
21. 作为用户，我希望通过“更多”重命名竞品组，以便修正组名而不影响竞品监控历史。
22. 作为用户，我希望重命名时仍受 trim、非空、64 字符和唯一性规则保护。
23. 作为用户，我希望删除竞品组时不会删除该组下的竞品，以便历史监控资料不会丢失。
24. 作为用户，我希望删除竞品组前看到竞品数量和转入未分组的后果，以便危险操作可理解、可确认。
25. 作为用户，我希望删除空型号也能成功，以便清理不再使用的型号。
26. 作为用户，我希望删除竞品组后竞品进入未分组，并保留快照、SKU 快照、变化事件、采集记录和监控状态。
27. 作为用户，我希望删除竞品组后 Dashboard 的历史变化仍然存在，只是之后按当前关系显示为未分组。
...
PageHeader
  标题：竞品分组
  说明：以我方商品型号命名竞品组，管理对应竞品。
  主要操作：新增竞品组
↓
竞品组卡片 Grid（组名使用商品型号，桌面端 2～3 列）
```
...
- 标题：新增竞品组；
- 字段标签：竞品组名称；
- placeholder：使用商品型号命名，例如 A19、X6；
...
| 竞品组 | 竞品组 |
| 新建分组 | 新增竞品组 |
| 输入分组名称 | 输入商品型号作为组名，例如 A19 |
| 分组已创建并已选中 | 竞品组已创建并已选中 |
| 请输入有效的分组名称 | 请输入有效的竞品组名称 |
| 该分组已存在 | 该竞品组已存在 |
...
空组时省略或改为“该组下暂无竞品”，仍显示取消和删除竞品组按钮。提交中禁止重复提交，失败保留 Dialog 和错误信息。
...
- 创建、重命名、删除竞品组原则上不接入 `COLLECTION_LOCK`；
- 删除组不会改变竞品的采集目标或历史记录；
- 组 CRUD 仍必须使用正常数据库事务；
...
- 无正式组且无未分组竞品：显示“还没有竞品组”，说明新增竞品组后可按商品型号归集对应竞品，并提供“新增竞品组”。
